...
void testCase()
{
    int start, end, c, r, cover_min, cover_max;
    scanI(start, end, c, r);
    if( start > end )
        swap(start, end);
    cover_min = c-r;
    cover_max = c+r;
    // if not covered at all, return entire range
    if( cover_max < start || end < cover_min)
    {
        outI(end-start, nl);
        return;
    }
    // covered at least a little bit
    // crop the covered range so that that it does not extend beyond the path
    cover_min = max(start, cover_min);
    cover_max = min(end, cover_max);
    //(time from start to covered range) + (time from end of covered range to the end)
    outI((cover_min - start) + (end - cover_max), nl);
}
