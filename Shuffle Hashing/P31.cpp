...
void scanS(const int max_size, string& str)
{
    char s[max_size];
    scanf("%s", s);
    str.assign(s);
...
template<typename t1, typename ... types>
void scanS(const int max_size, t1& str, types&...strings)
{
    scanS(max_size, str);
    scanS(max_size, strings...);
}
...
void testCase()
{
    int freq[26] = {0}, curr_freq[26] = {0};
    string a, b;
    scanS(101, a, b);
    // obtain the frequency array of short string
    REP(i,a.length())
    {
        ++freq[a[i]-'a'];
        ++curr_freq[b[i]-'a'];
    ...
    FOR(i,0,b.length()-a.length())
    {
        ...
        // decrease frequency of first character
        --curr_freq[b[i]-'a'];
        // increase frequency of the next character
        ++curr_freq[b[i+a.length()]-'a'];
    }
    puts("NO");
}
int main()
