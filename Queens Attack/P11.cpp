...
bool row_taken[MAXN], col_taken[MAXN], left_diag_taken[MAXN*2], right_diag_taken[MAXN*2];
// global variables
...
int main()
{
    ...
    for(int i = 0; i < q; ++i)
    {
        int row, col;
        scanI(row,col);
        if (row_taken[row] || col_taken[col] || left_diag_taken[row-col+n] || right_diag_taken[row+col])
        {
            cout << "NO\n";
            continue;
        }
        cout << "YES\n";
        row_taken[row] = col_taken[col] = left_diag_taken[row-col+n] = right_diag_taken[row+col] = true;
    }
