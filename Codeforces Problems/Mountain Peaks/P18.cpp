...
// https://codeforces.com/group/yg7WhsFsAp/contest/355493/problem/P18
// summing divisors: https://www2.math.upenn.edu/~deturck/m170/wk3/lecture/sumdiv.html
int main()
{
    fastio;
    #ifdef HENDRIX
    ...
    for(int block = 1; block <= n/2; ++block)
    {
        ...
        for(int start = 0; start < n; start += block)
        {
            ...
            {
                good_day |= (1 <= i && i <= n - 2 && h[i-1] < h[i] && h[i] > h[i+1]);
            }
            cnt_good += good_day;
        }
        if (cnt_good == n / block)
            answer = max(answer, cnt_good);
    }
    outI(answer, nl);
    #ifdef HENDRIX
