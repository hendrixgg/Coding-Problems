...
void testCase()
{
    scanf("%d", &n);
    int ones = 0, a = 0;
    bool greater_than_1 = false;
    REP(i,n){
        scanf("%d", &a);
        greater_than_1 |= a > 1;
        ones += !greater_than_1;
    }
    // if all are ones then First wins when there is an odd amount of ones
    // if not all ones then First wins when there is an even amount of ones
    printf("%s\n", ((ones == n)^(ones%2)) ? "Second" : "First");
}
