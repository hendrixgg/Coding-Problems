...
int f(vector<int>& a, int& max_sz)
{
    if( is_sorted(a.begin(), a.end()) )
        return max(max_sz, (int)a.size());

    vector<int> l_half(a.begin(), a.begin() + a.size()/2);
    vector<int> r_half(a.begin() + a.size()/2, a.end());

    return max(f(l_half, max_sz), f(r_half, max_sz));
}
