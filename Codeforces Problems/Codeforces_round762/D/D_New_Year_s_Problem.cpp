...
template <typename T>
void _dbarr(T *a, size_t sz)
{
  for (size_t i = 0; i < sz; i++)
    cerr << a[i] << " \n"[i == sz - 1];
}
template <typename T>
void _dbarr(vector<T> a, size_t sz)
{
  for (size_t i = 0; i < sz; i++)
    cerr << a[i] << " \n"[i == sz - 1];
}
...
namespace IO
{
  ...
  protected:
    ...
    OutputDevice() : output(buffer), end(buffer + BUFFER_SIZE + MAX_LENGTH), 
    separate(false), width(setWidth().value), fill(setFill().value), 
    base(setBase().value), precision(setPrecision().value), 
...
unique_ptr<IO::InputDevice> input;
unique_ptr<IO::OutputDevice> output;
...
// dummy variable
int x, lo, hi, mid;
// joy[i][j] = joy for friend j from shop i
vector<vector<int>> joy;
// global variables

bool can_get(int a)
{
    // must find a store with two gifts giving pleasure at least 'a''
    // it must be possible for each person to get pleasure at least 'a'
    vector<bool> can_get_a(friends, false);
    bool shop_with_two = false;
    for(const vector<int>& shop : joy)
    {
        int num_greater = 0;
        ...
        shop_with_two |= (num_greater >= 2);
    }
    return shop_with_two && *min_element(can_get_a.begin(), can_get_a.end());
}

void TestCase()
{
    read(shops, friends);
    // can go to all the shops 
    if(friends-1 >= shops)
    {
        joy.assign(1,vector<int>(friends, -INF));
        for(int i = 0; i < shops; ++i)
        {
            for(int j = 0; j < friends; ++j)
            {
                read(x);
                joy[0][j] = max(joy[0][j], x);
            }
        }
        alpha = *min_element(joy[0].begin(), joy[0].end());
        writeln(alpha);
        return;
    }
    // binary search over all possible values for alpha. range is [1,1e9]
    joy.assign(shops, vector<int>(friends));
    for(int i = 0; i < shops; ++i)
    {
        for(int j = 0; j < friends; ++j)
        {
...
int main()
{
    ...
    input.reset(new IO::InputFile(stdin, false));
    output.reset(new IO::OutputFile());

    int TTT; read(TTT);
    for(;TTT != 0; --TTT) TestCase();
