...
void TestCase()
{
    ...
    stack<int> extra;
    vector<ll> num_ops(n + 1, -1);
    ll ops_to_get_missing_nums = 0;
    for (int i = 0; i <= n; ++i)
    {
        // we were missing the previous number, (i-1).
        // So remove the greatest extra number, j,
        // from the stack and compute the difference between (i-1) and j.
        // This difference will contribute to the number of operations
        // required to get the missing numbers less than i.
        if (i > 0 && freq[i - 1] == 0)
        {
            if(extra.empty())
                break;
            int j = extra.top();
            extra.pop();
            ops_to_get_missing_nums += (i - 1) - j ;
        }
        // the number of operations to get a MEX of i =
        // (the number of operations to get the missing numbers less than i) + 
        // (number of i's that have to be incremented)
        num_ops[i] = ops_to_get_missing_nums + freq[i];
        // put extra numbers onto the stack
