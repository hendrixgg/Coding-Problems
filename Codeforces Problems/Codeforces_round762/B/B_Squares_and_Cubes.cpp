...
// global variables
// add the 0.5 because 1e9 as a floating point is stored as 9.999999e9
#define num_squares(n) (int)floor(sqrt(n + 0.5L))
#define num_cubes(n) (int)floor(cbrt(n + 0.5L))
#define num_both(n) (int)floor(pow(n+0.5L, 1.0L/6.0L))
void TestCase()
{
  read(n);
  // for(i = 1; i*i*i <= n; ++i)
  ...
  // writeln(likes.size());
  // likes.clear();
  writeln(num_squares(n) + num_cubes(n) - num_both(n));
}
